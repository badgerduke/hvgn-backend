...
module.exports.handler = async function (event, context) {
    ...
    try {
    const familyData = await getItem('Family', "FAMID, HUSB, WIFE", ddbKey('FAMID', familyId, "S"));
    ...
    if (Object.entries(familyData).length === 0) {
        ...
        if (father) {
            fatherPromise = getItem('Individual', "INDVID, GIVEN, SURN, SUFF, SEX, BIRTDATE, BIRTPLAC, DEATDATE, DEATPLAC", ddbKey('INDVID', father, "S"));
        }
        if (mother) {
            motherPromise = getItem('Individual', "INDVID, GIVEN, SURN, SUFF, SEX, BIRTDATE, BIRTPLAC, DEATDATE, DEATPLAC", ddbKey('INDVID', mother, "S"));
        }
        childrenPromise = queryByGsi('Children', 'familyIdGSI', 'FAMID = :a', {':a': {'S' : familyId}}, "_FREL, _MREL, CHILDID");
        ...
        for (let i = 0; i < childrenData.Items.length; i++) {
            childDataPromises.push(getItem('Individual', "INDVID, GIVEN, SURN, SUFF, SEX, BIRTDATE, DEATDATE", ddbKey('INDVID', childrenData.Items[i].CHILDID.S, "S")));
        }
...
let queryByGsi = function (tableName, gsiIndexName, keyExpression, expressionAttributeValues, projectionExpression) {
